...
class IpFabricDataSource(DataSource, Job):
    ...
    def sync_data(self):
        ...
        client = IpFabricClient(configs["IPFABRIC_HOST"], configs["IPFABRIC_API_TOKEN"])

        source = IPFabricDiffSync(job=self, sync=self.sync, client=client)
